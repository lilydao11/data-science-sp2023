...
-   What value of `fr` did you predict?
    ...
    I predicted a value of 0.5 because I am looking for the frequency that a Z value meets the criteria (lessthan or equal to zero). From looking at the graph, 50% of the data lies less than or equal to 0.

-   Did your prediction match your calculation?

    My prediction was very close to my calculation, as my calculation ended up being 0.51. The count changes every time I run the data which means the frequency also changes. That being said, it tends to average around 50%.
...
-   What value does `fr` appear to be limiting towards?
    ...
    fr appears to be limiting toward 0.95. This helped show that you need more counts to see a more accurate number of what a value like frequency is limiting toward.
...
-   What value is `fr` approaching?
    ...
    fr is approaching (or has reached) 0 because it is looking for a point where Z is exactly 2. The probability of a single point in a continuous distribution is zero.
...
-   What was your `fr` prediction in **q1**? How does `p0` compare?
    ...
    My fr prediction in q1 was 0.5 and that is exactly what my p0 is. This is because my p0 is the probability and not the frequency.
