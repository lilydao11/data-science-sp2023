...
```
    ...
    ## Rows: 1,000
    ## Columns: 2
    ## $ strength <dbl> 39918.81, 39364.60, 40235.09, 40285.44, 40126.05, 39992.84, 4…
    ## $ g        <dbl> 703.12496, 148.91317, 1019.40195, 1069.75846, 910.36253, 777.…
...
- Assuming your scopus is the probability of failure `POF` defined
  ...
  variability, or both?
  - Our estimate does not exhibit real variability because our POF
    relies on our distribution. Our distribution should not change,
    therefore our POF should not exhibit real variability. Furthermore,
    our estimate does not exhibit induced variability because we are
    assuming any strength measured is the true breaking strength. This
    means that the POF should not experience any variability from this.
...
- Can you *confidently* conclude that `POF < 0.03`? Why or why not?
  - No, when running the code several times, there are instances where
    ...
    failure could be greater than .03. Furthermore, our confidence
    interval does not account for the uncertainty from limited physical
    tests. Since we have uncertainty that is not accounted for, we
    cannot confidently conclude that POF \< 0.03.
...
- With the scopus as the `POF`, would uncertainty due to *Monte Carlo
  ...
  - There is no uncertainty in our POF due to Monte Carlo approximation.
...
- With the scopus as the `POF`, would uncertainty due to *limited
  ...
  - Uncertainty due to limited physical tests would be induced because
    we have limited information, which is not an issue from the samples
    themselves.
...
```
    ...
    ## 1 pof   0.000861    0.0175 0.0466   0.05 percentile
...
- Does the confidence interval above account for uncertainty arising
  ...
  - Yes, the confidence interval does account for uncertainty arising
    from limited physicals tests due to the resampling nature of the
    bootstrap method. It’s important to note that the resampling is from
    the limited sample we began with, but the bootstrap method is trying
    to account for the uncertainty of our limited sample.
- Can you confidently conclude that `POF < 0.03`? Why or why not?
