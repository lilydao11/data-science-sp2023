...
- It seems that commonly, the carat value of a diamond will be a
  multiple of 0.5, perhaps due to ease of understanding for those who
  aren’t knowledgeable with diamonds. As for the relationship with price
...
- With this graph, we can see that, just as before, as the carat value
  increases, the price tends to increase. Now we have the value of the
  cut, which has shown that the ideal cut is quite spread out, spanning
  from a fraction of a carat and a few hundred dollars to over two
  carats and over \$18k dollars. There seems to be a slight pattern with
  worse cuts lowering the value of the diamond, but there are still some
  diamonds with a fair cut that are going for over \$10k. As for making
  the graph, I find cut being the variable shown through an aesthetic is
  the best since it maintains the clear positive correlation between
  carat and price that the other graphs (cut vs carat or cut vs price)
  did not display.
