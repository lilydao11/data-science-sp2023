...
-   Assuming your scopus is the probability of failure `POF` defined above, does your estimate exhibit real variability, induced variability, or both?
    -   Our estimate exhibits real variability because there can be differences in the making of the samples but there can't be induced variability since we are assuming we are receiving the true strength values. Our strength values are also generated by a log curve which would ensure there is no induced variability.
...
-   Can you *confidently* conclude that `POF < 0.03`? Why or why not?
    -   No, when running the code several times, there are instances where .03 falls in the confidence interval meaning that the probability of failure could be greater than .03.
...
-   With the scopus as the `POF`, would uncertainty due to *Monte Carlo approximation* be induced or real?
    -   There is no uncertainty due to Monte Carlo approximation, but if there was, it would be real uncertainty, not induced certainty.
...
-   With the scopus as the `POF`, would uncertainty due to *limited physical tests* be induced or real?
    -   Uncertainty due to limited physical tests would be real, not induced.
...
-   Does the confidence interval above account for uncertainty arising from *limited physical tests* (`df_samples`)? Why or why not?
    -   No, while it does resample accounting for uncertainty in the values themselves, it does not account for the limited number of samples that are being processed through the data.
-   Can you confidently conclude that `POF < 0.03`? Why or why not?
