...
-   What is the mean strength of the material, approximately?
    -   I would approximate the mean strength of the material to be just below 40000 psi at around 39900 psi. That being said, this histogram is not a great way of estimating this value. This is especially difficult due to the small number of data points.
-   To what extent can you tell what shape the distribution of the data has?
    -   With 25 data points in our data set, we can't see the shape of the distribution with confidence. That being said, we do see a greater density of data in the left of our graph, leading me to believe that the graph is slightly skewed right. We would need more data points to verify the shape of the distribution.
-   Assuming the scopus is the strength of an individual part made from this aluminum alloy, is the observed variability real or induced?
    -   The variability is real because there are incosistencies in the manufacturing of parts that keeps one part from being identical to the other. Since we are to consider this the true breaking strength of the material, this implies that there was not induced variability.
...
-   Does this estimate satisfy `POF < 0.03`?
    -   This estimate does satisfy POF \< 0.03 because we got a POF of 0 with these samples.
-   Is this estimate of the probability of failure trustworthy? Why or why not?
    -   This estimate of POF is not trustworthy because we have so few samples. With a POF of around .03, we would expect around 3 in every 100 samples to fail. This means that we need to have significantly more samples than 25 to get a good idea of the actual POF for the material.
-   Can you confidently conclude that `POF < 0.03`? Why or why not.
    -   I cannot confidently conclude that POF for this material is less than 0.03. This is because we have so few samples. With only 25 samples, it isn't unlikely to get a POF of 0, but if we increased the samples to 1000, we would likely see some instances of failure.
...
-   Assuming your scopus is the probability of failure `POF` defined above, does your estimate exhibit real variability, induced variability, or both?
    -   Our estimate does not exhibit real variability because our POF relies on our distribution. Our distribution should not change, therefore our POF should not exhibit real variability. Furthermore, our estimate does not exhibit induced variability because we are assuming any strength measured is the true breaking strength. This means that the POF should not experience any variability from this.
-   Does this confidence interval imply that `POF < 0.03`?
    -   Yes, the lower end of our confidence interval is about POF = .00898 and the upper end is about POF = .0250. Our POF estimate is .017 making it so our entire interval is below .03.
-   Compare this probability with your estimate from q2; is it more or less trustworthy?
    -   This estimate of probability is more trustworthy because we are using more samples to understand the probability of failure.
-   Does the confidence interval above account for uncertainty arising from the *Monte Carlo approximation*? Why or why not?
    -   Yes, we can see it in the calculating of the value 'se' (standard error). This value takes into account the uncertainty that inherently comes from using a Monte Carlo approximation. When we multiply the standard error in the confidence interval calculations, we are taking the uncertainty into account.
-   Does the confidence interval above account for uncertainty arising from *limited physical tests* (`df_samples`)? Why or why not?
    -   No, we didn't use df_samples, we generated our own samples using the log fit estimates. That means that we did not account for limited physical tests in this confidence interval.
-   What could you do to tighten up the confidence interval?
    -   We could increase the amount of Monte Carlo simulations we do to tighten up the confidence interval.
-   Can you *confidently* conclude that `POF < 0.03`? Why or why not?
    -   No, when running the code several times, there are instances where .03 falls in the confidence interval meaning that the probability of failure could be greater than .03. Furthermore, our confidence interval does not account for the uncertainty from limited physical tests. Since we have uncertainty that is not accounted for, we cannot confidently conclude that POF \< 0.03.
...
-   How does this estimate compare with your Monte Carlo estimate above?
    -   It is around the average of the Monte Carlo estimates above. I had some instances of .017, .018, and .019 estimates for POF. This estimate is around the mean of these instances.
-   Does this estimate have any uncertainty due to *Monte Carlo approximation*? Why or why not?
    -   There is not any uncertainty due to Monte Carlo approximation because there is no Monte Carlo approximation in this calculation of the estimate.
...
-   Does this estimate have any uncertainty due to *limited physical tests*? Why or why not?
    -   There is uncertainty in our estimate due to limited physical tests because if we increased our tests, we would see an increase in the certainty of our estimate.
-   With the scopus as the `POF`, would uncertainty due to *limited physical tests* be induced or real?
    -   Uncertainty due to limited physical tests would be induced because we have limited information, which is not an issue from the samples themselves.
...
-   Does the confidence interval above account for uncertainty arising from *Monte Carlo approximation* of the POF? Why or why not?
    -   No, there is no uncertainty from a Monte Carlo approximation. This means that the confidence interval does not need to account for uncertainty arising from the Monte Carlo approximation.
-   Does the confidence interval above account for uncertainty arising from *limited physical tests* (`df_samples`)? Why or why not?
    -   Yes, the confidence interval does account for uncertainty arising from limited physicals tests due to the resampling nature of the bootstrap method. It's important to note that the resampling is from the limited sample we began with, but the bootstrap method is trying to account for the uncertainty of our limited sample.
-   Can you confidently conclude that `POF < 0.03`? Why or why not?
    -   No, .03 falls in the confidence interval, meaning we could get POFs greater than 0.3. This means that although we do see many values of POF in the range of .17-.19, we cannot conclude with confidence due to the upper end of this confidence interval.
